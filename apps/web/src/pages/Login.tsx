      ...
      <div className="w-full max-w-md rounded-[28px] border border-border bg-surface p-8 shadow-[0_28px_52px_rgba(16,18,20,0.14)]">
        ...
        <div className="mb-4 flex gap-2">
          <button
            type="button"
            onClick={() => setMode('app')}
            className={`rounded-full px-4 py-2 text-sm ${mode === 'app' ? 'bg-accent text-surface' : 'bg-surface2 text-text-muted'}`}
          >
            Operacao
          </button>
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm ${mode === 'admin' ? 'bg-accent text-surface' : 'bg-surface2 text-text-muted'}`}
          >
            Admin
          </button>
        </div>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <span>{mode === 'admin' ? 'Entrar no admin' : 'Entrar na operacao'}</span>
            </>
          )}
        </button>
        ...
        {ENABLE_DEV_LOGIN && (
          <button
            ...
            disabled={loading}
            className="mt-3 flex w-full items-center justify-center gap-3 rounded-full bg-accent px-6 py-4 font-medium text-surface transition-all duration-200 hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-50"
          >
            Entrar em modo dev
          </button>
        )}
