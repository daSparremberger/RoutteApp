...
export function LampAnimation({ height = 180 }) {
        ...
        const draw = () => {
            ...
            particlesRef.current.forEach((p) => {
                ...
                if (p.x < minX || p.x > maxX)
                    p.vx *= -1;
                if (p.y < height * 0.4 || p.y > height * 0.85)
                    p.vy *= -1;
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "absolute top-0 left-0 w-full pointer-events-none", style: { height } }));
}
