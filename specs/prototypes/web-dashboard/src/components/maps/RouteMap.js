...
export function RouteMap({ paradas, geojson }) {
    ...
    useEffect(() => {
        ...
        map.on('load', () => {
            ...
            if (valid.length > 1) {
                const bounds = valid.reduce((b, p) => b.extend([p.lng, p.lat]), new mapboxgl.LngLatBounds([valid[0].lng, valid[0].lat], [valid[0].lng, valid[0].lat]));
                map.fitBounds(bounds, { padding: 60, maxZoom: 14 });
            }
        });
        return () => map.remove();
    }, [paradas, geojson]);
    return _jsx("div", { ref: ref, className: "w-full h-80 rounded-xl overflow-hidden border border-surface2" });
}
